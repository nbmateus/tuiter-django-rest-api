...
def mainPostListView(request, username):
    if request.method == 'GET':
        queryset = Post.objects.filter(user__username=username, mainPost=None)
...
def likedPostsListView(request, username):
    if request.method == 'GET':
        ...
        serializer = PostSerializer(userProfile.postsLiked.all(), many=True)
        return Response(serializer.data)
...
def commentsListView(request, postId):
    ...
    if request.method == 'GET':
        comments = Post.objects.filter(mainPost=mainPost)
...
def indexMainPostListView(request):
    if request.method == 'GET':
        ...
        userProfile = UserProfile.objects.get(user=request.user)
        mainPostList = Post.objects.filter(user__in=userProfile.following.all(), mainPost=None).order_by('-timestamp')
